...
-- (We put this in a module so we don't clash with _>>=_ for IO below)
  ...
  -- FEEDBACK: this is not what we expect by do notations.
  -- more something like
  -- do x ← ea
  --    pure (f x)
...
-- FEEDBACK: this is just normal cong with unerase-≡
...
-- FEEDBACK: no need to use with here!
-- (1/1 MARK)
...
-- FEEDBACK: could be cleaner: you could have grouped
-- checks in auxiliary functions
...
-- and using map, fromList, and unlines would have been a lot cleaner!
-- (2/3 MARKS)
...
-- FEEDBACK: no need to pattern-match on w&h: just return x uniformly!
-- (0.5/1 MARK)
...
-- FEEDBACK: could be a bit cleaner
-- HARD (4/4 MARKS)
...
-- HARD, BECAUSE AGDA CAN BE CLUNKY (4/4 MARKS)
...
-- FEEDBACK: this is... unorthodox.
-- You could have reused the same fills by associating differently:
-- col ∥ (row ─ tile ─ row) ∥ col
...
-- FEEDBACK: giga chads use ∥, ─, vMirror and hMirror to generate and assemble the quadrants
-- (0/2 MARKS)
...
-- FEEDBACK: if you wrote this by hand, your pain threshold is way too high
-- (1/3 MARKS)
...
------------------------------------------------------------------------
-- Extend the project (3/15 MARKS) --------- 90,180,270 degree rotation, CLI for selecting file to save, color inversion
------------------------------------------------------------------------
